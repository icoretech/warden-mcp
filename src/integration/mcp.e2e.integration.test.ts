...
test(
  'mcp e2e: can initialize, list tools, and call keychain.status over /sse',
  { timeout: 120_000 },
  async (t) => {
    ...
    try {
      ...
      {
        const result = (
          genUserNoReveal.structuredContent as { result?: unknown }
        ).result;
        assert.ok(result && typeof result === 'object');
        const rec = result as {
          kind?: unknown;
      ...
      assert.equal(
        (hiddenField as Record<string, unknown>).value,
        '[REDACTED]',
      );
